...
class BatchPickingGroupMove(models.Model):
    ...
    group_product_id = fields.Many2one('product.product', string="Product", store=False)
    ...
    def action_apply_qties(self):
        ...
        if self.qty_done == sum(x.product_uom_qty for x in self.move_line_ids):
            ...
            moves_sorted = self.move_line_ids.sorted(lambda x: x.sale_order.date_order)
    ...
    def action_show_details(self):
        ...
        self.ensure_one()
        view = self.env.ref('stock_batch_picking_custom.view_stock_group_move_operations')
        ...
        return {
            ...
            'context': dict(
                ...
                show_lots_text=move_id.has_tracking != 'none' and self.batch_id.picking_type_id.use_create_lots and not self.batch_id.picking_type_id.use_existing_lots,
...
class StockBatchPicking(models.Model):
    ...
    def get_move_done(self):
        ...
        data = self.env['report.stock_batch_picking_custom.report_batch_picking_custom']._get_grouped_data(self)
    ...
    def sort_level_0(self, items, location_field="location_id"):
        ...
        res =  sorted(items, key=lambda item: (
    ...
    def action_get_moves_done(self):
        ...
        if not self.moves_all_count:
            self.get_move_done()
        view_tree = self.env.ref('stock_batch_picking_custom.view_stock_group_move_tree_operations')
        view_form = self.env.ref('stock_batch_picking_custom.view_stock_group_move_operations')
        return {
            'name': _('Detailed Operations'),
            'type': 'ir.actions.act_window',
            'view_type': 'form',
    ...
    def action_cancel_group(self):
        if all(not x.qty_done for x in self.move_grouped_ids):
            self.move_grouped_ids.mapped('move_line_ids').write({'qty_done': 0.00})
