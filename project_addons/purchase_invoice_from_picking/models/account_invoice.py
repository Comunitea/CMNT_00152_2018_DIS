...
class AccountInvoice(models.Model):
    ...
    def _prepare_supplier_invoice_line_from_stock_move(self, move):
        ...
        if rec:
            ...
            if rec.analytic_tag_ids:
                vals['analytic_tag_ids'] = rec.analytic_tag_ids.ids
    ...
    def onchange_receipt_picking_id(self):
        if self.receipt_picking_id:
            ...
            round_curr = self.receipt_picking_id.purchase_id.currency_id.round
            amount_untaxed = sum(
                l.purchase_price_subtotal for l in self.receipt_picking_id.move_lines)
            self.invoice_line_ids +=  self.env['account.invoice.line'].new({
                'name': "Importe calculado  " +
                        self.receipt_picking_id.display_name +
                        ": "
                        + str(amount_untaxed),
