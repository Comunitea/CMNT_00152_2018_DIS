odoo.define('dismac_ecommerce.customs_shop_js', function(require){
    'use strict';
    ...
    $(document).on('change','div.dropdown_sorty_by a.dropdown-toggle', function(){
        var test = $("div.dropdown_sorty_by a.dropdown-toggle")
    ...
    $(document).on('change', 'div#products_grid div.shop-filter-item_qty select.custom-select', function(){
        ...
        if($selEle.attr("data-shop_layout_type") === "0"){
            ...
            for(var i = 0; i < temp_attr_list.length; i++)
                $pagination_qty_change_form.append("<input type='hidden' name='attrib' value='" + temp_attr_list[i] + "'>");
            if($(".as-search form input[name='search']").val())
                $pagination_qty_change_form.append("<input type='hidden' name='search' value='" + $(".as-search form input[name='search']").val() + "'>");
            ...
            if(order_val && order_val !== '')
                $pagination_qty_change_form.append("<input type='hidden' name='order' value='" + order_val + "'/>");
            ...
            $.get("/shop/load_next_products", param_dict).then(function(data){
                if(data.trim()){
                    ...
                    $.getScript("/dismac_ecommerce/static/src/js/frontend_product_quick_view_js.js");
                }
                else{
    ...
    $(document).ready(function(){
        ...
        $("#my_cart").on("click", function(e){
            $.get("/shop/cart", {type: 'cart_lines_popup'}).then(function(data){
                if(data.trim()){
                    var $mini_cart_popup = $("#my_cart > .cart_lines_popup");
        ...
        $("#my_cart .my_cart_btn").on("click", function(e){
            var $target = $(this);
            $.get("/shop/cart", {type: 'cart_lines_popup'}).then(function(data){
                if(data.trim()){
        ...
        // footer cart //
        $("#footer_cart").on("click", function(e){
            $.get("/shop/cart", {type: 'cart_lines_popup'}).then(function(data){
                if(data.trim()){
                    var $mini_cart_popup = $("#footer_cart > .cart_lines_popup");
                    $mini_cart_popup.empty().append(data.trim()).addClass("show_mini_cart");
                }
            });
            e.stopPropagation();
        });
        $("#footer_cart .my_cart_btn").on("click", function(e){
            var $target = $(this);
            $.get("/shop/cart", {type: 'cart_lines_popup'}).then(function(data){
                if(data.trim()){
                    var $mini_cart_popup = $("header").find(".cart_lines_popup");
                    $mini_cart_popup.empty().append(data.trim()).addClass("show_mini_cart");
                }
            });
            e.stopPropagation();
        ...
        $(document).on("click", ".cart_lines_popup .m_c_close", function(){
