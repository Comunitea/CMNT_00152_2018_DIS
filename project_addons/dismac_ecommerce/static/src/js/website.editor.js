odoo.define('dismac_ecommerce.editor_js',function(require){
    'use strict';
    ...
    ajax.loadXML('/dismac_ecommerce/static/src/xml/change_progress.xml', core.qweb);
    ajax.loadXML('/dismac_ecommerce/static/src/xml/product_template.xml', core.qweb);
    ajax.loadXML('/dismac_ecommerce/static/src/xml/product_slider_popup.xml', core.qweb);
    ...
    options.registry.theme_progressbar = options.Class.extend({
        ...
        change_progress: function(type,value) {
            var self = this;
            self.$modal = $(qweb.render("dismac_ecommerce.change_progress_modal"));
    ...
    options.registry.product_brand_slider = options.Class.extend({
        brand_slider_configure: function(type,value) {
            ...
            if(type=="click"  || value===undefined){
                self.$modal = $(qweb.render("dismac_ecommerce.brand_slider_block"));
                self.$modal.appendTo('body');
                self.$modal.modal();
    ...
    options.registry.product_slider_actions = options.Class.extend({
        product_slider_configure: function(previewMode, value){
            ...
            if(previewMode === false || previewMode === "click"){
                self.$modal = $(qweb.render("dismac_ecommerce.p_slider_popup_template"));
                ...
                self.$modal.on('change', self.$modal.find("select[name='s_tab_layout'],select[name='m_tab_layout']"), function(e){
                    ...
                    self.$modal.find("form .p_slider_sample_view img.snip_sample_img").attr("src", "/dismac_ecommerce/static/src/img/snippets/" + $sel_ele.val() + ".png");
