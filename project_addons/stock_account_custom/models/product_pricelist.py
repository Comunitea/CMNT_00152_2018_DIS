...
class Pricelist(models.Model):
    ...
    def _compute_price_rule(self, products_qty_partner, date=False,
        ...
        if uom_id:
            ...
            products = [item[0].with_context(uom=uom_id) for item in
        ...
        for p in products:
            categ = p.categ_id
            type = p.cost_ratio_id
            if type:
                type_ids[type.id] = True
            while categ:
        ...
        self._cr.execute(
            ...
            'AND (item.cost_ratio_id IS NULL OR item.cost_ratio_id = any(%s)) '
            'AND (item.pricelist_id = %s) '
            'AND (item.date_start IS NULL OR item.date_start<=%s) '
            'AND (item.date_end IS NULL OR item.date_end>=%s)'
            'ORDER BY item.applied_on, item.min_quantity desc, categ.parent_left desc',
            (prod_tmpl_ids, prod_ids, categ_ids, type_ids, self.id, date,
            date))
        ...
        for product, qty, partner in products_qty_partner:
            ...
            if qty_uom_id != product.uom_id.id:
                try:
                    qty_in_product_uom = self.env['product.uom'].browse(
                        [self._context['uom']])._compute_quantity(qty,
                                                                  product.uom_id)
                except UserError:
            ...
            if suitable_rule and suitable_rule.compute_price != 'fixed' and suitable_rule.base != 'pricelist':
                price = product.currency_id.compute(price, self.currency_id,
                                                    round=False)
...
class PricelistItem(models.Model):
    ...
    base = fields.Selection(
        selection_add=[('pricelist_cost', 'Pricelist Cost')],
    ...
    applied_on = fields.Selection([
        ...
        ('2_1_product_type', ' Product Type'),
        ('1_product', 'Product'),
        ('0_product_variant', 'Product Variant')], "Apply On",
        default='3_global', required=True,
        help='Pricelist Item applicable on selected option')

    cost_ratio_id = fields.Many2one('product.price.ratio', 'Product Type',
                                    help="Product ranking to get reference"
