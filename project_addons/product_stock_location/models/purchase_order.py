...
class PurchaseOrder(models.Model):
    ...
    def _get_destination_location(self):
        ...
        if (
            ...
            and not self.dest_address_id
            and str(res) in destination_location.parent_path.split("/")
        ):
...
class PurchaseOrderLine(models.Model):
    ...
    def _prepare_stock_moves(self, picking):
        destination_location = (
            self.product_id.property_stock_location
            or self.product_id.categ_id.property_stock_location
            or False
        )
        if (
            destination_location
            and self.order_id.picking_type_id.code == "incoming"
        ):
