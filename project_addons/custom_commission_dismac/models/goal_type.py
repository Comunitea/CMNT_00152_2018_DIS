...
class GoalType(models.Model):
    ...
    global_units = fields.Boolean("Compute units globally", default=True)
    ...
    # Objetivo ventas mobiliario mes
    mob_sale_type_id = fields.Many2one("sale.order.type", "Furniture Sale Type")
    ...
    def get_sale_goal_commission(self, info_data):
        ...
        domain = [("id", "in", self.by_sale_ids.ids), ("goal_per", "<=", per)]
        ...
        if line:
            ...
            note += (
                _("Rule applied -> From percentage goal: %s") % line.goal_per
            )
        return commission, note
    ...
    def get_margin_goal_commission(self, sale_type_info):
        ...
        domain = [("id", "in", self.by_margin_ids.ids), ("coef", "<=", coef)]
    ...
    def get_mob_commission(self, sale_types_info, min_data):
        ...
        if self.mob_sale_type_id and sale_types_info.get(
            self.mob_sale_type_id, {}
        ):
    ...
    def get_inf_commission(self, sale_types_info, min_data):
        ...
        if self.info_sale_type_id and sale_types_info.get(
            self.info_sale_type_id, {}
        ):
    ...
    def get_min_customers_commission(self, sale_types_info, min_data={}):
        ...
        if customers and len(customers) >= min_data["min_customers"]:
            ...
            com2, note2 = self.get_num_customers_commission(
                sale_types_info, min_data
            )
            commission += com2
            note += sep + note2 + sep

            # Calculo nuevos clientes
            com2, note2 = self.get_new_customers_commission(
                sale_types_info, min_data
            )
            commission += com2
            note += sep + note2 + sep
    ...
    def get_commission(self, sale_type_info, min_data={}):
        ...
        if self.type == "sale_goal":
            ...
            commission, note = self.get_min_customers_commission(
                sale_type_info, min_data
            )
        return commission, note
