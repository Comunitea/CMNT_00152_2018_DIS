...
class PLPCLine(models.TransientModel):
    ...
    partner_price_unit = fields.Float(string='Unit Price', required=True, digits=dp.get_precision('Product Price'))
...
class PurchaseLinePartnerChangeWzd(models.TransientModel):
    ...
    initial_amount = fields.Float(string="Inicial", digits=dp.get_precision('Product Price'))
    partner_amount = fields.Float(string="Propuesto", digits=dp.get_precision('Product Price'))
    line_ids = fields.Many2many("plpc.line")
    partner_ids = fields.Many2many('res.partner')
    purchase_order_id = fields.Many2one('purchase.order', 'Pedido de compra encontrado')
    ...
    def default_get(self, field_list):
        ...
        if active_ids:
            pol_ids = self.env['purchase.order.line'].browse(active_ids).filtered(lambda x: x.state=='draft')
            for pol in pol_ids:
        ...
        suplier_ids = self.env['product.supplierinfo'].search(domain).mapped('name')
    ...
    def apply_partner(self):
        if self.partner_id:
            ...
            for line in self.line_ids:
                pol_val = {'product_qty': line.product_qty,
    ...
    def apply_order(self):
        if self.partner_id:
            ...
            for line in self.line_ids.filtered(lambda x: x.partner_id in self.partner_ids and x.partner_price_unit > 0):
                pol_val = {'product_qty': line.product_qty,
                           'name': line.product_id.display_name,
                           'partner_id': self.partner_id.id,
                           'product_id': line.product_id.id,
