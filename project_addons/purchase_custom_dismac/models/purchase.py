...
class PurchaseOrder(models.Model):
    ...
    needed_for_free_delivery = fields.Float(
        ...
        compute="_check_min_delivery_amount", store=True)
...
class PurchaseBillUnion(models.Model):
    ...
    def name_get(self):
        ...
        for doc in self:
            ...
            if doc.reference:
                name += ' - ' + doc.reference
            amount = doc.amount
            if doc.purchase_order_id and \
                    doc.purchase_order_id.invoice_status == 'no':
                amount = 0.0
            name += ': ' + formatLang(
    ...
    def _name_search(self, name, args=None, operator='ilike',
        ...
        purchases = self.env['purchase.order'].search(
            [('supplier_picking_ref', operator, name)])._ids
        args = args or []
        domain = []
        if name:
