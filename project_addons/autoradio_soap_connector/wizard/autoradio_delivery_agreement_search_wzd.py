...
class AutoradioDeliveryAgreementWizard(models.TransientModel):

    _name = "autoradio.delivery.agreement.search.wzd"
    ...
    def autoradio_delivery_agreement_search(self):
        if not self.dates:
            self.env['autoradio.config'].WSBuscadorConformeEntrega(codigoCentro=self.center_code, reference=self.reference)
        elif self.dates and self.date_from and self.date_to:
            self.env['autoradio.config'].WSBuscadorConformeEntregaEntreFechas(date_from=self.date_from.strftime('%Y-%m-%dT%H:%M:%S'), date_to=self.date_to.strftime('%Y-%m-%dT%H:%M:%S'))
        else:
