...
class PricelistImportWzd(models.TransientModel):
    ...
    def import_pricelist(self):
        ...
        for nline in range(3, sh.nrows):
            ...
            row[41] = datetime.datetime(
                *xlrd.xldate_as_tuple(date_start, book.datemode))

            date_end = sh.cell_value(rowx=nline, colx=42)
            row[42] = datetime.datetime(
                *xlrd.xldate_as_tuple(date_end, book.datemode))
            ...
            if row[0]:
                ...
                if not product_pricelist_obj:
                    product_pricelist_obj = self.create_new_product_pricelist(
                        row_vals['name'],
                        row_vals['date_start'],
                        row_vals['date_end'])

                # Buscamos si existe algún producto (product.product)
                # donde el campo ean13_str contenga el ean
                product_obj = self.env['product.product'].search(
                    [('catalogue_code', '=', row_vals['catalogue_code']),
                     ('catalogue_code', '!=', False)])

                if product_obj and len(product_obj) == 1:
                    if row_vals['c1'] and row_vals['pvp1']:
                        self.create_new_product_pricelist_item(
                            row_vals['c1'], row_vals['pvp1'],
                            product_obj, product_pricelist_obj,
                            row_vals['date_start'], row_vals['date_end'])

                    if row_vals['c2'] and row_vals['pvp2']:
                        self.create_new_product_pricelist_item(
                            row_vals['c2'], row_vals['pvp2'], product_obj,
                            product_pricelist_obj, row_vals['date_start'],
                            row_vals['date_end'])

                    if row_vals['c3'] and row_vals['pvp3']:
                        self.create_new_product_pricelist_item(
                            row_vals['c3'], row_vals['pvp3'], product_obj,
                            product_pricelist_obj, row_vals['date_start'],
                            row_vals['date_end'])

                    if row_vals['c4'] and row_vals['pvp4']:
                        self.create_new_product_pricelist_item(
                            row_vals['c4'], row_vals['pvp4'], product_obj,
                            product_pricelist_obj, row_vals['date_start'],
                            row_vals['date_end'])
                else:
                    if len(product_obj) != 1:
                        error_msg = _(
                            "There are {} products with the "
                            "catalogue_code: {}. "
                            "You may check this out.\n").format(
                                len(product_obj), row_vals['catalogue_code'])
                        self.create_new_error_line(
                            error_msg, product_pricelist_obj.id)

        return self.view_product_pricelist(product_pricelist_obj.id)

    def create_new_product_pricelist_item(
            self, quantity, price, product_obj, product_pricelist_obj,
            date_start, date_end):
    ...
    def create_new_error_line(self, error_msg, id):
        self.env['product.pricelist.import.error'].create({
            'import_id': id,
            'error_msg': error_msg
        })
    ...
    class PricelistImportError(models.Model):
        ...
        import_id = fields.Many2one(
            'product.pricelist', 'Pricelist Import', ondelete="cascade")
